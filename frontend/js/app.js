...
// ============ CONFIGURATION ============
const CONFIG = {
  // Utilise l'URL de l'API depuis la variable d'environnement (Vercel/Netlify)
  // ou fallback vers Render pour la production
  API_URL: (() => {
    // 1. Variable d'environnement (définie sur Vercel/Netlify)
    if (typeof process !== 'undefined' && process.env.NEXT_PUBLIC_API_URL) {
      return process.env.NEXT_PUBLIC_API_URL.replace(/\/$/, ''); // remove trailing slash
    }
    // 2. Localhost pour dev
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
      return 'http://localhost:3001/api';
    }
    // 3. Production – fallback direct vers Render (pas de proxy Vercel)
    return 'https://merdeeee.onrender.com/api';
  })(),
};
