...
const CONFIG = {
  ...
  // Production API base (without /api)
  get productionBase() {
    return 'https://mami-bazooka.onrender.com';
  },
  ...
  // Build full image URL; on localhost, rewrite production URLs to localhost
  getImageUrl(path) {
    if (!path) return 'https://via.placeholder.com/300?text=No+Image';
    // Already absolute URL
    if (path.startsWith('http')) {
      // If on localhost and URL points to production, rewrite to localhost
      if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        if (path.includes('mami-bazooka.onrender.com')) {
          return path.replace('https://mami-bazooka.onrender.com', 'http://localhost:3001');
        }
        if (path.includes('http://mami-bazooka.onrender.com')) {
          return path.replace('http://mami-bazooka.onrender.com', 'http://localhost:3001');
        }
      }
      return path;
    }
    ...
    return `${this.getImageBaseUrl()}/${cleanPath}`;
  }
};

// Allow overriding API_URL via ?api= query param (useful for dev)
if (typeof window !== 'undefined' && window.location.search) {
  try {
    const params = new URLSearchParams(window.location.search);
    const apiOverride = params.get('api');
    if (apiOverride) {
      const clean = apiOverride.replace(/\/$/, '');
      CONFIG.API_URL = `${clean}/api`;
      console.log('API_URL overridden to', CONFIG.API_URL);
    }
  } catch (e) {
    // ignore
  }
...
function createProductCard(product) {
  ...
  card.style.transform = 'translateY(20px)';
   ...
   if (product.images && product.images.length > 0) {
     const imgUrl = CONFIG.getImageUrl(product.images[0]);
  ...
  if (prodImagesInput) {
    prodImagesInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        const token = localStorage.getItem('bazooka_token');
        if (!token) {
          showToast('Vous devez être connecté pour uploader des images', 'error');
          return;
        }
        ...
        for (const file of files) {
          ...
          if (data && data.url) {
            ...
            throw new Error('Pas d\'URL reçue du serveur');
          }
        }
        ...
        if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
          showToast('Serveur inaccessible. Vérifiez que le backend est démarré sur localhost:3001', 'error');
        } else {
...
function renderProductImagePreviews() {
  ...
  container.innerHTML = '';
   ...
   uploadedProductImages.forEach((url, index) => {
     ...
     const fullUrl = CONFIG.getImageUrl(url);
...
function triggerAvatarUpload() {
  const input = document.getElementById('avatar-upload-input') || document.getElementById('avatar-input');
  if (input) input.click();
}

async function uploadAvatar() {
  const fileInput = document.getElementById('avatar-upload-input') || document.getElementById('avatar-input');
  if (!fileInput) return;
  ...
  try {
    const token = localStorage.getItem('bazooka_token');
    if (!token) {
      showToast('Vous devez être connecté pour uploader une image', 'error');
      return;
    }
    ...
    avatarEls.forEach(el => { if (el) el.src = data.url; });
...
function showToast(message, type = 'info') {
  ...
  if (!container) {
    ...
    container.className = 'fixed bottom-8 right-8 z-[120] space-y-2';
